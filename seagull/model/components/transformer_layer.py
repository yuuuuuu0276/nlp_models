...
class TransformerLayer(Module):
    ...
    def forward(
        ...
        """
        maybe_norm_input_embeddings = self.maybe_apply_layer_norm(input_embeddings, current_layer_norm_application="pre")   # MHA "pre"
        out, masked_attn_probs = self.mha(maybe_norm_input_embeddings, padding_mask, use_kv_cache=use_kv_cache)
        out = out + input_embeddings

        maybe_norm_out = self.maybe_apply_layer_norm(out, current_layer_norm_application="post")  # MHA "post"

        maybe_norm_in = self.maybe_apply_layer_norm(maybe_norm_out, current_layer_norm_application="pre")   # FFN "pre"
        out_ffn = self.ffn(maybe_norm_in)
        out_ffn = out_ffn + maybe_norm_out

        maybe_norm_ffn = self.maybe_apply_layer_norm(out_ffn, current_layer_norm_application="post")  # FFN "post"
        return maybe_norm_ffn, masked_attn_probs
