...
class SeagullLM(Module):
    ...
    def forward(
        ...
        # call seagull model
        all_output = self.seagull(input_ids, position_ids, padding_mask, use_kv_cache, 
          return_output_at_all_layers, return_attentions)

        # handle output accordingly (i.e. tuple or not etc.)
        if return_attentions:
          output = all_output[0][-1]
        else:
          output = all_output[-1]

        # project into logits accordingly
        if self.weight_tying:
          logits = output @ self.seagull.embedding.token_embedding.weight.data.T
        else:
          logits = self.lm_head(output)

        return logits, all_output
