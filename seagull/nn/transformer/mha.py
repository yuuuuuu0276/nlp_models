...
class MultiHeadAttention(Module):
    ...
    def masked_attention_probs(
        ...
        d = query.shape[-1]
        out = torch.einsum("bhld,bhid -> bhli", query, key)
        out = out / torch.sqrt(torch.tensor(d))
        if padding_mask is not None:
          padding_mask = padding_mask.unsqueeze(1)
          padding_mask = padding_mask.unsqueeze(1) 
          out = out.masked_fill(padding_mask, float("-inf"))
        out = self._apply_causal_mask(out)
        out = softmax(out, dim = -1, dropout_proba = self.attn_dropout_proba, training = self.training)
        return out
    ...
    def attention(masked_attn_probs: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
        """See: https://pages.github.coecis.cornell.edu/cs4740/hw4-fa23/seagull.nn.transformer.mha.html."""
        out = torch.einsum("bhli, bhid -> bhld", masked_attn_probs, value)
        return out
